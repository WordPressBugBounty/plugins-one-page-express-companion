...
add_filter(
	'cloudpress\template\page_content',
	function ( $content ) {
...
add_action(
	'cloudpress\template\load_assets',
	function ( $companion ) {
...
add_action(
	'cloudpress\customizer\preview_scripts',
	function ( $customizer ) {
		$ver = $customizer->companion()->version;
		wp_enqueue_script(
			$customizer->companion()->getThemeSlug() . '_preview-handle',
			$customizer->companion()->themeDataURL() . '/preview-handles.js',
			array( 'cp-customizer-preview' ),
			$ver
		);
	}
);
...
add_action(
	'cloudpress\customizer\global_scripts',
	function ( $customizer ) {
		$ver = $customizer->companion()->version;
		wp_enqueue_script(
...
function one_page_header_css() {
	$headerContentCSS = \OnePageExpress\Companion::getThemeMod(
		'onepage_builder_header_content_css',
		array()
	);

	$headerContentCSS = array_merge(
		array(
	...
	<style>
		...
		.header-description-right h1 {
			...
			text-align: <?php echo strip_tags( $mappedSettings['title']['text-align'] ); ?>;
		...
		.header-description-right .header-subtitle {
			...
			text-align: <?php echo strip_tags( $mappedSettings['subtitle']['text-align'] ); ?>;
...
function one_page_inner_header_css() {
	$headerContentCSS = \OnePageExpress\Companion::getThemeMod(
		'onepage_builder_inner_header_content_css',
		array()
	);
...
add_action(
	'cloudpress\companion\deactivated\one-page-express',
	function ( $companion ) {
...
add_filter(
	'cloudpress\companion\front_page_content',
...
add_filter(
	'cloudpress\customizer\control\content_sections\data',
	function ( $data ) {
		$categories = array(
			'overlapable',
			'about',
...
add_filter(
	'cloudpress\customizer\control\content_sections\category_label',
...
add_action(
	...
	function () {
		...
		<style data-name="overlap">
			@media only screen and (min-width: 768px) {
				.header-homepage {
					padding-bottom: <?php echo  strip_tags( $margin ); ?>;
				}
				...
				.homepage-template .page-content div[data-overlap]:first-of-type>div:first-of-type {
					margin-top: -<?php echo strip_tags( $margin ); ?>;
